...
때문에 New IO라는 이름의 채널 기반의 새로운 입출력 시스템을 2002년 방식을 도입합니다. 
...
항상 스트림 닫기
더 이상 필요하지 않을 때 스트림을 닫는 것은 매우 중요 합니다.
 오류가 발생하더라도 두 스트림이 모두 닫히도록 
 finally블록을 사용하는 것이 매우 중요 합니다. 이 방법은 심각한 리소스 누출을 방지하는 데 도움이됩니다. (아니면 try-resource-with를 사용하는 것도 좋습니다.)
위 소스는 실제로는 피해야하는 일종의 저수준 IO입니다. 
...
Java에서는 유니 코드 규칙을 사용하여 문자 값을 저장합니다.
 ...
 하지만 Character 스트림을 사용하는 프로그램은 프로그래머의 추가 노력없이 로컬 문자 집합에 자동으로 적응하고 인코딩(UTF-16)까지 해준다.
...
```
CopyCharacters사용 FileReader하고 FileWriter대신 입력 및 출력 FileInputStream하고 FileOutputStream를 사용합니다. 
Character 스트림은 바이트 스트림의 "래퍼"입니다. 
Character 스트림은 바이트 스트림을 사용하여 물리적 IO를 수행하는 또한, Character 스트림은 문자와 바이트 간의 변환을 처리합니다.
요구 사항을 충족하는 사전 패키징 된 문자 스트림 클래스가 없을 땐 문자 스트림을 만들 수 있습니다.
Character 스트림은 일반적으로 단일 문자보다 더 큰 단위로 발생합니다. 
하나의 공통 단위는 줄입니다 : 끝에 종결자가있는 문자열입니다. 
